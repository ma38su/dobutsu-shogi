...
開発サーバーでは、対局画面の「AIのつよさ」の下に開発用の探索情報を表示します。探索局面数、生成手数、置換表ヒット数とヒット率、αβカット回数、到達深度、探索時間、Workerとの通信を含む応答全体の時間、1秒あたりの探索局面数、評価値、選択手を確認できます。iPadの縦・横画面でもこの欄を表示し、実機固有の遅さが探索とWorker応答のどちらにあるかを切り分けられます。この表示は本番ビルドには出ません。
...
「ふつう」「つよい」「とてもつよい」について、平均・中央値・95パーセンタイル・最大の探索時間と平均探索局面数を出力します。端末間の絶対性能の比較ではなく、同じ環境で実装変更の前後を比較する用途を想定しています。
