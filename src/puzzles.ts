...
export const PUZZLES:PuzzleDefinition[]=[
  ...
  puzzle('starter-7','starter','たての もちごま','まっすぐ進む持ち駒を、ライオンの横へ打って詰ませよう',1,[gote('lion'),null,sente('hen'),gote('chick'),gote('chick'),null,null,null,sente('hen'),sente('lion'),sente('chick'),gote('giraffe')],['giraffe','chick']),
  puzzle('starter-8','starter','ちゅうおうへ ななめ','斜めの駒を中央へ動かし、離れた場所から王手をかけよう',1,[gote('lion'),null,null,sente('elephant'),null,sente('chick'),null,sente('lion'),sente('elephant'),null,null,sente('giraffe')]),
  puzzle('starter-9','starter','守りを 取って','王手をじゃまする守り駒を取りながら、逃げ道も閉じよう',1,[gote('elephant'),gote('lion'),null,null,null,gote('giraffe'),null,sente('hen'),sente('hen'),sente('hen'),sente('lion'),null],[],['elephant']),
  puzzle('starter-10','starter','いっぽ前の 王手','まっすぐ動く駒を一歩進めて、前と横の逃げ道を押さえよう',1,[gote('lion'),gote('chick'),null,null,null,null,sente('giraffe'),sente('lion'),null,null,sente('elephant'),sente('giraffe')]),
  ...
  puzzle('stepup-4','stepup','取って パワーアップ','相手の駒を取りながら成り、広がった動きで次の王手へつなごう',3,[gote('lion'),gote('chick'),sente('elephant'),null,sente('chick'),sente('elephant'),null,null,null,null,sente('lion'),null],['chick'],['chick']),
  puzzle('stepup-5','stepup','ぞうで 道をあける','斜めの駒で守りを取り、相手を呼び込んで最後の王手を決めよう',3,[gote('lion'),null,gote('hen'),sente('elephant'),gote('hen'),sente('hen'),sente('elephant'),sente('chick'),null,sente('lion'),sente('hen'),null],['elephant'],['elephant']),
  puzzle('stepup-6','stepup','ななめから まっすぐ','斜めの王手で移動先を決め、最後は持ち駒でまっすぐ押さえよう',3,[sente('giraffe'),gote('giraffe'),null,sente('chick'),null,gote('lion'),null,null,sente('elephant'),sente('elephant'),sente('lion'),sente('chick')],['giraffe']),
  puzzle('stepup-7','stepup','守りを もちごまに','守りの駒を取って逃げ道を減らし、持ち駒の王手で仕上げよう',3,[null,gote('lion'),sente('elephant'),null,gote('giraffe'),null,sente('lion'),sente('giraffe'),gote('chick'),null,null,null],['giraffe','elephant'],['chick','chick']),
  puzzle('stepup-8','stepup','取り合いの あと','相手に取り返させる手順を読み、その直後の持ち駒で詰ませよう',3,[gote('lion'),gote('hen'),null,gote('giraffe'),sente('giraffe'),null,sente('giraffe'),sente('giraffe'),sente('lion'),null,sente('chick'),sente('chick')],['giraffe','chick']),
  puzzle('stepup-9','stepup','上から おさえて打つ','盤上の駒で上から追い、逃げた先へ小さな持ち駒を打とう',3,[null,sente('giraffe'),null,null,null,gote('lion'),null,sente('chick'),sente('elephant'),sente('lion'),sente('hen'),null],['chick']),
  puzzle('stepup-10','stepup','打って 取らせる','持ち駒をあえて取らせ、空いた場所へ別の駒を進めよう',3,[gote('hen'),gote('lion'),null,null,null,null,sente('lion'),sente('hen'),null,sente('chick'),sente('elephant'),null],['giraffe','elephant'],['chick','chick']),
  ...
  puzzle('challenge-4','challenge','三つの駒の リレー','持ち駒、盤上の長い駒、小さな駒へ王手を受け渡そう',5,[null,null,gote('lion'),sente('giraffe'),null,null,null,null,sente('chick'),sente('lion'),sente('giraffe'),null],['giraffe']),
  ...
  puzzle('challenge-6','challenge','取った駒で ふさぐ','最初に取った駒を持ち駒として使い、次の逃げ道を封鎖しよう',5,[null,gote('lion'),sente('elephant'),null,gote('giraffe'),gote('giraffe'),null,sente('chick'),sente('hen'),sente('lion'),sente('elephant'),null]),
  puzzle('challenge-7','challenge','打って取り また打つ','持ち駒で追い、守りを取って増えた駒を最後にもう一度打とう',5,[null,gote('lion'),null,sente('giraffe'),null,gote('giraffe'),null,sente('lion'),null,null,null,sente('chick')],['giraffe'],['elephant','chick']),
  puzzle('challenge-8','challenge','成り駒を おとりに','駒を取りながら成り、その成り駒を取らせて斜めの連続王手へつなごう',5,[gote('elephant'),gote('lion'),null,sente('chick'),null,null,null,sente('elephant'),sente('elephant'),null,sente('lion'),sente('elephant')],['elephant'],['giraffe']),
  ...
  puzzle('challenge-10','challenge','取ったひよこを 打つ','相手の小さな駒を取り、持ち駒にしてすぐ次の王手へ活用しよう',5,[gote('lion'),gote('chick'),sente('giraffe'),gote('chick'),null,sente('chick'),null,sente('chick'),sente('chick'),sente('elephant'),sente('lion'),null],['giraffe'],['chick']),
]
